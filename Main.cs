...
namespace HotBrakes
{
    public class Main : BaseScript
    {
        ...
        internal async Task HeatBrakes()
        {
            ...
            {
                //You can remove this last line, since it's for testing pourposes.
                //Screen.DisplayHelpTextThisFrame($"{hotBrakes}.");
                if (lastCar == veh)
                {
                    if (veh.Speed > 5f && veh.CurrentGear != 0 && Game.IsControlPressed(0, Control.VehicleBrake) || Game.IsControlPressed(0, Control.VehicleHandbrake))
                    {
                        if (hotBrakes > 10000)
                        {
